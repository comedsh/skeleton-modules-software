...
public class OrderMasterResultDTO implements Serializable {
	...
	public OrderMasterResultDTO(OrderMaster master) {
		...
		if(master.getOrderHeaders() != null && !master.getOrderHeaders().isEmpty()) {
			this.orderNoList = new ArrayList<String>(master.getOrderHeaders().size());
			for (OrderHeader oh : master.getOrderHeaders()) {
				this.orderNoList.add(oh.getOrderNo());
			}
		}
