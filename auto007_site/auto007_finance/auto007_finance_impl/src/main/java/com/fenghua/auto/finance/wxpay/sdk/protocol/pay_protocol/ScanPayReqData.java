...
public class ScanPayReqData {
    ...
    //每个字段具体的意思请查看API文档
    private String appid = "";				//是——微信分配的公众账号ID
    private String mch_id = "";				//是——微信支付分配的商户号
    private String device_info = "";		//否——终端设备号(门店号或收银设备ID)，注意：PC网页或公众号内支付请传"WEB"
    private String nonce_str = "";			//是——随机字符串，不长于32位。推荐随机数生成算法
    private String sign = "";				//是——签名
    private String body = "";				//是——商品或支付单简要描述
    private String detail = "";				//否——商品名称明细列表
    private String attach = "";				//否——附加数据，在查询API和支付通知中原样返回，该字段主要用于商户携带订单的自定义数据
    private String out_trade_no = "";		//是——商户系统内部的订单号,32个字符内、可包含字母, 其他说明见商户订单号
    private String fee_type = "CNY"; 		//否——符合ISO 4217标准的三位字母代码，默认人民币：CNY，其他值列表详见货币类型
    private int total_fee = 0;				//是——订单总金额，只能为整数，详见支付金额，单位分
    private String spbill_create_ip = "";	//是——APP和网页支付提交用户端ip，Native支付填调用微信支付API的机器IP。
    private String time_start = "";			//否——订单生成时间，格式为yyyyMMddHHmmss，如2009年12月25日9点10分10秒表示为20091225091010。其他详见时间规则
    private String time_expire = "";		//否——订单失效时间，格式为yyyyMMddHHmmss，如2009年12月27日9点10分10秒表示为20091227091010。其他详见时间规则
    										//注意：最短失效时间间隔必须大于5分钟
    private String goods_tag = "";			//否——商品标记，代金券或立减优惠功能的参数，说明详见代金券或立减优惠
    private String notify_url = "";			//是——接收微信支付异步通知回调地址
    private String trade_type = "";			//是——取值如下：JSAPI，NATIVE，APP，WAP,详细说明见参数规定
...
//    private String auth_code = "";			//这个是扫码终端设备从用户手机上扫取到的支付授权号，这个号是跟用户用来支付的银行卡绑定的，有效期是1分钟
    private String sdk_version;				//sdk的版本号
...
//        setAuth_code(authCode);
    ...
    public ScanPayReqData(String device_info, String body, String detail, String attach,
			String out_trade_no, int total_fee, String spbill_create_ip, String notify_url, String trade_type,
			String product_id) {
		super();
//		setSdk_version(Configure.getSdkVersion());
    ...
    public ScanPayReqData(String device_info, String body, String detail, String attach,
			String out_trade_no, int total_fee, String spbill_create_ip, String notify_url, String trade_type,
			String product_id, String timeStart,String timeExpire) {
		super();
//		setSdk_version(Configure.getSdkVersion());
